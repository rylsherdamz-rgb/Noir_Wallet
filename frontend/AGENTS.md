...
`prepareTransaction` doesn't auto-populate `sorobanAuth` entries for contracts using `wallet.require_auth()`. Transactions fail with `sceAuth` error. The simulation returns no auth entries, so the prepared transaction lacks authorization.
